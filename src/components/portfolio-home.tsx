...
function ArtifactCard({project,index}:{project:Project;index:number}) {
  const type = project.slug === "foobar" ? "Technical prototype" : "Personal build";
...
export function PortfolioHome({projects,testimonials,writing}:{projects:Project[];testimonials:Testimonial[];writing:WritingItem[]}) {
  ...
  const artifacts = useMemo(() => ["foobar","cycle-synced"].map(slug => projects.find(project => project.slug === slug)).filter(Boolean) as Project[], [projects]);
